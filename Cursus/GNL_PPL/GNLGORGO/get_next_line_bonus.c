...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/07 08:59:28 by jormoral          #+#    #+#             */
/*   Updated: 2024/06/10 19:37:39 by jormoral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*sbuffer[FD_SETSIZE];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	sbuffer[fd] = ft_read_buffer(sbuffer[fd], fd);
	if (!sbuffer[fd])
		return (NULL);
	line = ft_read_line(sbuffer[fd]);
	sbuffer[fd] = ft_updatebuffer(sbuffer[fd]);
	return (line);
}
