...
/*   Updated: 2024/11/26 19:26:33 by sbenitez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	game->img_enemy = mlx_texture_to_image(
			game->mlx, mlx_load_png("textures/enemy.png"));
...
void	render_tile(t_game *game, int i, int j)
{
	...
	if (game->map->data[i][j] == '1')
	{
		...
		mlx_image_to_window(game->mlx, game->img_collect,
			j * TILE_SIZE, i * TILE_SIZE);
	else if (game->map->data[i][j] == 'R')
		mlx_image_to_window(game->mlx, game->img_enemy,
			j * TILE_SIZE, i * TILE_SIZE);
